...
class IsOwnerSkillKeywordOrReadOnly(BasePermission):
    """
    Object-level permission to only allow owners of an object to edit it.
    Assumes the model instance has an `skill_keyword.skill.profile.user.id` attribute.
    """
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # Instance must have an attribute named `skill_keyword.skill.profile.user.id`.
        return obj.skill_keyword.skill.profile.user.id == request.user.id
