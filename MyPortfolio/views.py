...
from profile_settings.models import Pricing, Message, EmailApp
...
def index(request):
    ...
    projects = (
        Project.objects.filter(profile=profile.id).order_by("date").reverse()[0:4]
    )  # 4 Recent Projects
    ...
    # 3 Recent Articles
    articles = (
        Article.objects.filter(status="Publish").order_by("date_created").reverse()[0:3]
    )
    return render(
        request,
...
def contact_me(request):
    if request.is_ajax():
        ...
        if email_settings.count() > 0:
            ...
            # Send Email to Admin
            name = (
                str(request.POST["first_name"]) + " " + str(request.POST["last_name"])
            )
            ...
            part = MIMEText(
                email_body.format(name=name, email1=email1, message1=message1), "plain"
            )
...
def auth_login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            try:
                return JsonResponse(
                    {"success": "User logged in", "redirect": request.GET["next"]},
                    status=200,
                )
            except Exception:
